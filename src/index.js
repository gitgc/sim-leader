...
app.post('/leaderboard/:id/profile-picture', isAuthorized, upload.single('profilePicture'), async (req, res) => {
  try {
    const { id } = req.params;
...
function isRaceExpired(raceDate) {
  ...
  const midnightPST = new Date(racePST.getFullYear(), racePST.getMonth(), racePST.getDate() + 1, 8, 0, 0); // Next day midnight in UTC
...
app.get('/race-settings', async (req, res) => {
  try {
    ...
    console.error('Error fetching race settings:', error);
...
app.post('/race-settings/circuit-image', isAuthenticated, upload.single('circuitImage'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded' });
    }
...
app.delete('/race-settings/circuit-image', isAuthenticated, async (req, res) => {
  try {
    let settings = await RaceSettings.findOne();
    if (!settings) {
