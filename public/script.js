...
async function checkAuthStatus() {
    ...
    if (authOperationInProgress) {
        console.log('Authentication operation already in progress, skipping auth status check');
        return;
    }
...
async function logout() {
    ...
    if (authOperationInProgress) {
        console.log('Authentication operation already in progress');
        return;
    }
...
function renderLeaderboard() {
    ...
    leaderboardElement.innerHTML = drivers.map((driver, index) => {
...
async function handleAddDriver(event) {
    ...
    if (!driverData.driverName || isNaN(driverData.points) || driverData.points < 0) {
...
function getNotificationColor(type) {
    switch (type) {
        ...
        case 'error': return 'linear-gradient(45deg, #f44336, #da190b)';
...
async function loadRaceSettings() {
    try {
        ...
        console.error('Error loading race settings:', error);
    }
}
...
async function handleSettingsUpdate(event) {
    ...
    try {
        ...
        console.error('Error updating race settings:', error);
        showNotification('Failed to update race settings. Please try again.', 'error');
    }
}

async function clearNextRace() {
    if (!currentUser) {
        showNotification('Please sign in to clear race settings.', 'error');
        return;
    }
    
    const confirmed = confirm('Are you sure you want to clear the next race? This will remove all race information including location, date, description, and circuit image.');
    ...
    try {
        ...
        try {
            ...
            // Ignore circuit image deletion errors (e.g., if no image exists)
            console.log('No circuit image to delete or deletion failed:', imageError);
        }
...
async function deleteCircuitImage() {
    if (!currentUser) {
    ...
    try {
        ...
        console.error('Error deleting circuit image:', error);
